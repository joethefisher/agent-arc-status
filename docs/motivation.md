...
- A multi-agent system uses the Protocol as the lingua franca for delegated work — Agent A sends a task to Agent B, B emits arc-status events, A consumes them to know when to follow up.
...
- Teams overload `milestone` for every internal step (anti-pattern §5.3). The cure is the heuristic: *if the consumer asked right now "how is it going?", what's the answer?*
- Teams skip the silence backstop. The cure is structural: pair Protocol adoption with a watchdog that fires `heartbeat` automatically.
- Teams emit `done` before the deliverable is actually visible. The cure is one of discipline; the spec calls it out as a conformance violation, but adopters need to enforce it culturally too.
